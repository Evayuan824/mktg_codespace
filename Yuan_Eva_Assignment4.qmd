...
# use .select when you only need a few columns, use .with_columns when you need all columns in the dataframe 

travelers = email.select(
  pl.col("age"), pl.col("avg_distance"), pl.col("n_purchase"),
  pl.when(pl.col("n_purchase")>5)
  .then(pl.lit("Yes"))
  .otherwise(pl.lit("No"))
  .alias("frequent_purchaser"),
...
# px is the name of the package, scatter is the function, travelers is a function from above 
...
q5 = px.histogram(
   travelers,
   x = "age",
   facet_row = "frequent_purchaser",
   color = "frequent_purchaser",
   title = "Age by Purchase Frequency"
)
...
q7 = px.bar(
  travelers
  .group_by("age_group")
  .agg(pl.col("avg_distance")
  .mean()
  .alias("mean_avg_distance")
  ),
  x = "age_group",
  y = "mean_avg_distance",
  color="age_group"
