...
observation = (
    observation_data
        ...
        .agg([
            pl.col("days_since").min().alias("most_recent_transaction (R)"), # R
            pl.col("Customer.ID").count().alias("number_of_transactions (F)"), # F
            pl.col("Total").sum().alias("total_revenue (M)") # M
        ])
)
```
...
customer_data = (
    observation
    .join(purchaser, on="Customer.ID", how="left")
    .fill_null(0)
    )
...
R_hist = px.histogram(customer_data, x = 'most_recent_transaction (R)')
R_hist.show()


F_hist = px.histogram(customer_data, x = 'number_of_transactions (F)')
F_hist.show()



M_hist = px.histogram(customer_data, x = 'total_revenue (M)')
M_hist.show()
...
customer_data_transformed = customer_data.with_columns([
    pl.col("total_revenue (M)").log().alias("log_total_revenue"),
    pl.col("number_of_transactions (F)").log().alias("log_number_of_transactions"),
])
...
features = ['most_recent_transaction (R)', 'log_number_of_transactions', 'log_total_revenue']
...
numeric_features = ['most_recent_transaction (R)', 'log_number_of_transactions', 'log_total_revenue']
...
# Top 100 customers with highest predicted probability
top_customers = (
    customer_test
    .sort("pred_probability", descending=True)
    .head(num_customers_targeted)
)

# How many of those customers actually made a purchase
actual_purchasers = top_customers["made_purchase"].sum()

# Calculate revenue and cost
total_profit = actual_purchasers * per_purchaser_profit
total_cost = num_customers_targeted * per_person_cost

# ROMI formula
romi = (total_profit - total_cost) / total_cost

# Print results
print("Actual purchasers in top 100:", actual_purchasers)
print("Total profit from purchasers:", total_profit)
