...
df_example = email.filter(pl.col("age") > 70 ) # just old people
df_example

df_example2 = df_example.sort("age", descending=True) # old people descending by age 
df_example2

df_example3 = df_example2.head() # top 5 oldest people 
df_example3

dfe0 = email.filter(pl.col('age')>70).sort('age', descending= True).head() # still a data frame | this tells us who the 5 oldest people are in one line instead of having 3 lines typed out like above (method chaining)
...
# this code would not run because new iines matter in python. lines cannot start with . (.sort, .head, etc.)
```
...
dfe2 = (
    ...
    .sort('age', descending= True)
    .head()
) # as long as the code is in parentheses, new lines do not matter 
...
# .equals function checks if these dataframes are identical 
...
avg_price_by_class = (
    email.group_by("is_second_class") # used .group_by because it says "grouped by" in the prompt
    .agg( # chaining .agg right after .group_by
        pl.col("price")
        .mean()
    )
)
...
total_revenue_success = (
    email.filter(pl.col("success") == True)
    .select(
        pl.col("total_revenue")
        .sum()
        )
...
avg_price_multi_group = (
    email.group_by(pl.col("success"), pl.col("is_second_class"), maintain_order=True) # maintain_order=True makes it so that the rows show up in the same order every time # technically pl.col is not needed in every occasion but it's good practice to know 
    .agg(
        pl.col("price")
        .mean()
    )
...
age_group_data = (
    ...
    .group_by("age_group")
    .agg(
        pl.len(), # returns the "length" of how many people are in a specific group 
        pl.col("ID"),
...
# forward filling strategy (uses number prior to null)
# backward filling strategy (uses number after null)
...
# .over column specific subsetting 
